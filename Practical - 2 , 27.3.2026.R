...
# 2. Classify orders by value.
sales_data$ValueCategory <- ifelse(sales_data$FinalAmount > 40000, "High",
                                   ifelse(sales_data$FinalAmount > 20000, "Medium", "Low"))
sales_data

# 3. Print revenue using for loop.
for(i in 1:nrow(sales_data)) {
  print(paste("Order", sales_data$OrderID[i], "Revenue:", sales_data$Revenue[i]))
}

# 4. Cumulative revenue using while loop.
i <- 1
cum_rev <- 0
...
# 5. High price + high quantity 
subset(sales_data, Price > 30000 & Quantity > 7)
